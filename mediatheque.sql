...
SELECT * FROM genres;
SELECT films_titre AS 'Titre', films_resume AS 'Resume', films_annee AS 'Annee', films_affiche AS 'Affiche', films_duree AS 'Duree', real_nom AS 'Realisateur', GROUP_CONCAT(DISTINCT acteurs_nom) AS 'Acteurs', GROUP_CONCAT(DISTINCT genres_nom) AS 'Genres'
    FROM films
...
SELECT films_titre AS Titre, films_resume AS Resumé, films_annee AS Annee, films_affiche AS Affiche, films_duree AS Durée, real_nom AS Realisateur, GROUP_CONCAT(DISTINCT acteurs_nom) AS Acteurs, GROUP_CONCAT(DISTINCT genres_nom) AS Genres
    FROM films
	LEFT JOIN realisateurs ON films_real_id=real_id
    LEFT JOIN films_acteurs ON films_id=fa_films_id
    LEFT JOIN acteurs ON fa_acteurs_id=acteurs_id
    LEFT JOIN films_genres ON films_id=fg_films_id
    LEFT JOIN genres ON genres_id=fg_genres_id
	GROUP BY films_titre, films_resume, films_annee, films_affiche, films_duree, real_nom -- Le GROUP BY doit être suffisamment détaillé
    HAVING Titre LIKE '%300%'
		OR Resumé LIKE '%Action%'
        OR Annee LIKE '%Action%'
        OR Affiche LIKE '%Action%'
        OR Durée LIKE '%Action%'
        OR Realisateur LIKE '%Action%'
        OR Acteurs LIKE '%Action%'
        OR Genres LIKE "%Action%";
    
SELECT count(films_id) AS 'nbrFilms'
	FROM films;
SELECT LOCATE('Oblivion', 'rouge');
    
DROP TABLE IF EXISTS admin;
CREATE TABLE admin (
    adm_id INT PRIMARY KEY AUTO_INCREMENT,
    adm_nom VARCHAR(25) NOT NULL,
    adm_prenom VARCHAR(25) NOT NULL,
    adm_mdp text NOT NULL,
    adm_mail VARCHAR(50) CHECK (adm_mail LIKE '%@%') -- Utiliser les expressions régulières
);

INSERT INTO admin(adm_nom, adm_prenom, adm_mdp, adm_mail) VALUES 
('Sturam','Nicolas', MD5(1234), 'nicolas.sturam@hotmail.be');
SELECT * FROM films; 
SELECT * FROM realisateurs;

INSERT INTO films VALUES (NULL,'Le Seigneur des Anneaux',
    null,
    2001,'interstellar.jpg',169,27);
